...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoice, onClose }) => {
  const { addInvoice, updateInvoice, clients } = useData();
  const [formData, setFormData] = useState({
    number: invoice?.number || '',
    date: invoice?.date || new Date().toISOString().split('T')[0],
    dueDate: invoice?.dueDate || '',
    clientId: invoice?.clientId || '',
    items: invoice?.items || [{ id: '1', description: '', quantity: 1, unitPrice: 0, tax: 0, total: 0 }],
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setFormData(prev => ({ ...prev, status: e.target.value as 'draft' | 'sent' | 'paid' | 'overdue' | 'cancelled' }))}
          >
